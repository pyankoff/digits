...
def data_input(file, rows, columns):
    X = np.zeros((rows, columns))
    y = np.zeros((rows, 1))
    with open(file, 'rb') as csvfile:
        datareader = csv.reader(csvfile, delimiter=',')
        #row_count = sum(1 for row in datareader) - 1        -TODO
        #column_count = len(datareader.next())               -TODO
        i = 0
        for row in datareader:
            try:
                ...
                continue
            if i < rows:
                y[i] = int(int_list.pop(0))
                X[i] = np.array(int_list)
                i += 1
            else:
                break
    return X, y
...
X, y = data_input('digits\\train.csv', 5000, 784)
m, n = X.shape
