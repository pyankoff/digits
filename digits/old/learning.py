...
def randomInit(dim):
    return np.random.random(dim) * 2 * 0.12 - 0.12
...
def nnCostFunction(nn_params,
    ...
    a3 = forwardProp(Theta1, Theta2, X)[0]
    ...
    #regularization
    Theta1reg, Theta2reg = regularization(Theta1, Theta2)
...
def nnGrad(nn_params,
    ...
    for t in xrange(m):
        # step 1
        a3, z2, a2, a1 = forwardProp(Theta1, Theta2, X[t,:].reshape((1, input_layer_size)))
        # step 2
        yt = yv[t, :].reshape(out_layer_size,1)
        delta3 = a3.T - yt;         
        # step 3
        delta2 = np.dot(Theta2[:, 1:].T, delta3) * sigmoidGradient(z2.T)
        # step 4
